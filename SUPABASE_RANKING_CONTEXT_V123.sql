...
-- Ejecutar en Supabase > SQL Editor antes de usar la v123.
-- No usa cache ni tablas temporales persistentes: todo se calcula en CTEs en memoria.

create or replace function public.roque_get_student_ranking_context_v123(
  p_login text
)
returns jsonb
language plpgsql
...
grant execute on function public.roque_get_student_ranking_context_v123(text) to anon, authenticated;
...
set search_path = public
as $$
  select public.roque_get_student_ranking_context_v123(p_login);
$$;
